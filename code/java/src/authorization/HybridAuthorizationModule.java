...
public class HybridAuthorizationModule implements AuthorizationModule {
    private Connection con = null;

    public HybridAuthorizationModule(String databaseUsername, String databasePassword) {
        try {
    ...
    public boolean checkAccess(String subject, String action, String object) {
        try {
            String sql = "SELECT Policy  FROM Authorization JOIN Roleassignment ON " +
            "Authorization.Role = Roleassignment.Role " +
                    "WHERE Policy IS NOT NULL AND UserName = ? AND Action = ? ";
...
//            <policy><rule> subject.name = object.name</rule></policy>
    ...
    public static void main(String[] args) {
        HybridAuthorizationModule hyac = new HybridAuthorizationModule("root", "123456");
        System.out.println(hyac.checkAccess("user1", "read", "object1"));
        System.out.println(hyac.checkAccess("user2", "read", "object1"));
        System.out.println(hyac.checkAccess("user3", "read", "object1"));
